    ...
    <Fluid class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex gap-4">
          <IpInput inputId="ip-input" v-model:ip="serverInEdit.ip" label="Server Ip Address" />
          <FloatLabel class="float-label-spacer" variant="in">
            <InputText inputId="country-input" v-model="serverInEdit.country_code" :max="2" />
            <label for="country-input">Server Country Code</label>
          </FloatLabel>
        </div>
        <div class="flex gap-4">
          <FloatLabel class="float-label-spacer" variant="in">
...
import { getEmptyServer, type Server } from '@/api/server/schema';
...
const serverInEdit = ref<Server>(getEmptyServer());

watch(
  () => props.server,
  () => {
    if (!props.server) {
      serverInEdit.value = getEmptyServer();
    }
    else {
